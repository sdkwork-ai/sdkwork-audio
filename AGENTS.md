...
This is the SDKWork Audio Application root. The application provides professional audio capabilities including speech synthesis, transcription, translation, sound effects, voice cloning, and real-time audio processing.
...
- `sdkwork.app.config.json`: application configuration manifest.
...
- Local directories to inspect first when relevant: `crates/`, `packages/`, `sdks/`, `tests/`.
...
Load language specs only when touched: Rust uses `RUST_CODE_SPEC.md`, TypeScript/Node uses `TYPESCRIPT_CODE_SPEC.md`.
